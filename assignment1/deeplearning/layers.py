...
def affine_forward(x, w, b):
  ...
  for i in range(1,len(x.shape)):
      D*=x.shape[i]
  x_1d=np.reshape(x,(N,D))
  out=np.matmul(x_1d,w)+b
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, w, b)
...
def affine_backward(dout, cache):
  ...
  for i in range(1,len(x.shape)):
      D*=x.shape[i]
  x_1d=np.reshape(x,(N,D))
  dx_1d=np.matmul(dout,w.T)
  dw=np.matmul(x_1d.T,dout)
  db=np.sum(dout,axis=0)
  dx=np.reshape(dx_1d,x.shape)
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx, dw, db
...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    sample_var = 1.0/N * np.sum(sq, axis = 0)
...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = gamma * (dout - dbeta / N - x_hat * dgamma / N) / sqrtvar
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_forward_naive(x, pool_param):
  ...
  Ht = 1 + (H - pool_height) / float(stride)
  Wt = 1 + (W - pool_width)/ float(stride)
  Ht=int(Ht)
  Wt=int(Wt)
